...
export default function Edit({ track: initialTrack }) {
    const [formData, setFormData] = useState({
        name: initialTrack.name,
        location: initialTrack.location,
        length: initialTrack.length,
        description: initialTrack.description,
        difficulty: initialTrack.difficulty
    });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await axios.put(`/api/tracks/${initialTrack.id}`, formData);
            router.visit(route('tracks.index'));
        } catch (err) {
            if (err.response?.data?.errors) {
                setErrors(err.response.data.errors);
            } else {
                setErrors({ submit: 'Une erreur est survenue lors de la mise à jour de la piste' });
            }
        } finally {
            setLoading(false);
        }
    ...
    return (
        <AuthenticatedLayout header={<h1 className="text-2xl font-bold text-gray-800 dark:text-white">Modifier la piste</h1>}>
